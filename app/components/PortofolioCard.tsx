...
const PortoCard = ({ porto, layout = "grid" }: PortoCardProps) => {
  if (layout === "list") {
    return (
      <div className="bg-[#191919] rounded-xl overflow-hidden group">
        <div className="grid grid-cols-1 md:grid-cols-2">
          <div className="relative">
            <Link href={`/blog/${porto.slug}`}>
              <Image
                src={porto.image || "/assets/imgs/blog/placeholder.jpg"}
                alt={porto.title}
                width={400}
                height={400}
  ...
  return (
    <div className="group flex flex-col h-full">
      <div className="overflow-hidden mb-6">
        <Link href={`/blog/${porto.slug}`}>
          <Image
            src={porto.image || "/placeholder.jpg"}
            alt={porto.title}
            width={800}
            height={600}
      ...
      <div className="flex flex-col flex-grow px-1">
        <div className="text-sm text-gray-400 mb-3 uppercase tracking-wider">
          <span>{formatDate(porto.createdAt.toString())}</span>
        </div>
