...
import { updatePorto } from "@/lib/action";
import { useFormState } from "react-dom";
import { SubmitButton } from "../button";
import type { Portofolio } from "@/lib/generated/prisma";
import type { PortoFormState, CKEditorConfig } from "@/types/blog";
...
const EditPorto = ({ portofolio }: { portofolio: Portofolio }) => {
  const UpdatePortoWithId = updatePorto.bind(null, portofolio.id);
  const [state, formAction] = useFormState<PortoFormState | null, FormData>(
    UpdatePortoWithId,
    null
  );
  ...
  return (
    <div className="container mt-10">
      ...
      <p className="text-center text-gray-600 mb-10">
        Edit the post by updating the form below.
      </p>
      <form action={formAction}>
        ...
        <div className="mt-5">
          ...
          <input
            type="text"
            name="client"
            defaultValue={portofolio.client}
            className="border w-full px-5 py-3 focus:outline-bg_vidi rounded-md"
          />
          <div aria-live="polite" aria-atomic="true">
            <p className="text-sm text-red-500 mt-2">
        ...
        <div className="mt-5">
          ...
          <input
            ...
            name="startDate"
            defaultValue={
              portofolio.startDate
                ? new Date(portofolio.startDate).toISOString().split("T")[0]
                : ""
            }
            className="border w-full px-5 py-3 focus:outline-bg_vidi rounded-md"
          />
        ...
        <div className="my-5">
          ...
          <select
            id="category"
            name="category"
            defaultValue={portofolio.category}
          ...
          <select
            id="status"
            name="status"
            defaultValue={portofolio.status}
            className="border focus:outline-bg_vidi px-5 py-2 rounded-md"
          >
          ...
          <label className="mr-5">Tag</label>
          <select
            id="tag"
            name="tag"
            defaultValue={portofolio.tag}
            className="border focus:outline-bg_vidi px-5 py-2 rounded-md"
          >
            <option value="News">News</option>
            <option value="Tips">Tips</option>
